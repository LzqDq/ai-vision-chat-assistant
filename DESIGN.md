...
| CS-05 | **批量处理**：音频分段批量发送，减少请求次数 | 10-20% 网络开销 | ✅ 已实现 |
| CS-06 | **本地预处理**：简单的图像预处理在本地完成 | 10-15% 计算量 | ✅ 已实现 |
...
#### CS-05: 音频批量处理
- **实现位置**: `audio.js`, `main.js`
- **配置参数**: `AUDIO_BATCH_INTERVAL` (默认3000ms)
- **工作原理**: 录音过程中每3秒自动发送一批音频片段，而非等录音结束一次性发送
- **效果**: 减少约10-20%的网络开销，避免大文件传输超时

#### CS-06: 本地图像预处理
- **实现位置**: `main.js` (`preprocessImageSync` 函数)
- **配置参数**: `imageQuality` (默认60%), `maxImageWidth` (默认800px)
- **工作原理**: 使用Canvas API在前端进行图片缩放和JPEG压缩后再上传
- **效果**: 减少约10-15%的计算量和带宽消耗
